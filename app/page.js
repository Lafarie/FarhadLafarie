...
function Home() {
  ...
  const toggleMobileMenu = (event) => {
    // Stop propagation to prevent immediate handleOutsideClick triggering
    event.stopPropagation();
    setIsMobileMenuOpen((prev) => !prev);
  };

  useEffect(() => {
    // Add event listeners only once (not on every render when menu state changes)
  ...
  // We still need to update handleOutsideClick when isMobileMenuOpen changes
  useEffect(() => {
    // This effect ensures handleOutsideClick has access to current isMobileMenuOpen value
  }, [isMobileMenuOpen]);
    ...
    <>
      ...
      <div id="home" className="mt-12 pt-20 min-h-screen bg-black bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 flex flex-col items-center justify-center p-4 md:p-8">
        ...
        <div className={`text-center max-w-3xl mx-auto transition-all duration-1000 ease-in-out delay-300 transform ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          ...
          <p className="text-lg md:text-xl text-gray-700 dark:text-gray-300 mb-3 font-medium typing-cursor">{typedText}</p>
        ...
        <div className="w-full max-w-4xl bg-white border border-gray-200 rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700 mt-8 transition-all hover:shadow-xl fade-slide-up stagger-4">
          <div className="sm:hidden">
            <button onClick={toggleMobileMenu} className="p-2 text-gray-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:focus:ring-gray-600" aria-label="Toggle navigation">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={isMobileMenuOpen ? "M6 18L18 6M6 6l12 12" : "M4 6h16M4 12h16M4 18h16"}></path>
              </svg>
            </button>
            {isMobileMenuOpen && (
              <ul id="mobile-menu" className="absolute top-16 left-0 w-full bg-white shadow-lg dark:bg-gray-800 z-50">
                <li>
                  <button
                    onClick={() => {
          ...
          {/* Change the id here to avoid conflict with the mobile menu */}
          <ul className="hidden text-sm font-medium text-center text-gray-500 divide-x divide-gray-200 rounded-lg sm:flex dark:divide-gray-600 dark:text-gray-400">
            <TabButton id="stats-tab" activeTab={activeTab} tabId="stats" onClick={handleTabClick} onClose={handleMenuOptionClick}>
              Statistics
            </TabButton>
            <TabButton id="about-tab" activeTab={activeTab} tabId="about" onClick={handleTabClick} onClose={handleMenuOptionClick}>
              Services
            </TabButton>
            <TabButton id="faq-tab" activeTab={activeTab} tabId="faq" onClick={handleTabClick} onClose={handleMenuOptionClick}>
              FAQ
            </TabButton>
          </ul>
            ...
            <form className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="fade-slide-in stagger-1">
                  ...
                  <input ref={emailRef} type="email" id="email" className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-3 transition-all dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-red-500 dark:focus:border-red-500" placeholder="name@email.com" required onFocus={(e) => e.target.classList.add("scale-in")} />
                </div>
                <div className="fade-slide-right stagger-2">
                  <label htmlFor="subject" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                    Subject
                  </label>
                  <input ref={subjectRef} type="text" id="subject" className="block p-3 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 shadow-sm focus:ring-red-500 focus:border-red-500 transition-all dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-red-500 dark:focus:border-red-500" placeholder="Let me know how I can help you" required onFocus={(e) => e.target.classList.add("scale-in")} />
                </div>
              </div>
              <div className="fade-slide-up stagger-3">
                ...
                <textarea ref={messageRef} id="message" rows="6" className="block p-3 w-full text-sm text-gray-900 bg-gray-50 rounded-lg shadow-sm border border-gray-300 focus:ring-red-500 focus:border-red-500 transition-all dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-red-500 dark:focus:border-red-500" placeholder="Share your thoughts or questions..." onFocus={(e) => e.target.classList.add("scale-in")}></textarea>
              </div>
              <div className="flex justify-center md:justify-end fade-slide-up stagger-4">
                <button type="button" onClick={handleSendMessage} className="py-3 px-8 text-sm font-medium text-center text-white rounded-lg bg-red-600 hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 transition-all transform hover:scale-105 shadow-md hover:shadow-lg dark:bg-red-500 dark:hover:bg-red-600 dark:focus:ring-red-700 group">
                  <span className="flex items-center">
                    Send Message
